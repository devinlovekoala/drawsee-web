...
const orderPortsForElement = (type: CircuitElementType, ports: Endpoint[], portIdToNet: Record<string, string>) => {
  switch (type) {
    ...
    case CircuitElementType.DIODE_SCHOTTKY: {
      ...
      return [portIdToNet[getEndpointKey(first)], portIdToNet[getEndpointKey(second)]];
    }
...
const pickProp = (el: CircuitDesign['elements'][number], key: string): unknown => {
  const props = (el.properties || {}) as Record<string, unknown>;
  return props[key];
};
...
export const buildNetlist = (design: CircuitDesign) => {
    ...
    const findCandidate = () => {
      for (const el of design.elements) {
        if (
          ...
          el.type === CircuitElementType.AC_SOURCE ||
          el.type === CircuitElementType.PULSE_SOURCE ||
          el.type === CircuitElementType.PWM_SOURCE ||
          el.type === CircuitElementType.SINE_SOURCE
        ) {
  ...
  design.elements.forEach(el => {
    ...
    switch (el.type) {
      ...
      case CircuitElementType.AC_SOURCE:
      case CircuitElementType.SINE_SOURCE: {
        const vo = pickProp(el, 'vo') ?? 0;
        const va = pickProp(el, 'va') ?? pickElementValue(el, '1');
        const freq = pickProp(el, 'freq') ?? '1k';
        const td = pickProp(el, 'td') ?? 0;
        const df = pickProp(el, 'df') ?? 0;
        lines.push(`V${label} ${nets[0]} ${nets[1]} SIN(${vo} ${va} ${freq} ${td} ${df})`);
        break;
      }
      case CircuitElementType.PULSE_SOURCE: {
        const v1 = pickProp(el, 'v1') ?? 0;
        const v2 = pickProp(el, 'v2') ?? 5;
        const td = pickProp(el, 'td') ?? '1ns';
        const tr = pickProp(el, 'tr') ?? '1ns';
        const tf = pickProp(el, 'tf') ?? '1ns';
        const pw = pickProp(el, 'pw') ?? '5ns';
        const per = pickProp(el, 'per') ?? '10ns';
        lines.push(`V${label} ${nets[0]} ${nets[1]} PULSE(${v1} ${v2} ${td} ${tr} ${tf} ${pw} ${per})`);
        break;
      }
      case CircuitElementType.PWM_SOURCE: {
        const freqHz = Number(pickProp(el, 'freqHz') ?? 1000);
        const dutyRaw = Number(pickProp(el, 'duty') ?? 0.5);
        const duty = Math.min(1, Math.max(0, Number.isFinite(dutyRaw) ? dutyRaw : 0.5));
        const per = 1 / (Number.isFinite(freqHz) && freqHz > 0 ? freqHz : 1000);
        const pw = per * duty;
        const vlow = pickProp(el, 'vlow') ?? 0;
        const vhigh = pickProp(el, 'vhigh') ?? 5;
        lines.push(`V${label} ${nets[0]} ${nets[1]} PULSE(${vlow} ${vhigh} 0 1n 1n ${pw} ${per})`);
        break;
      }
      case CircuitElementType.DIODE:
        ...
        break;
      case CircuitElementType.DIODE_ZENER: {
        const model = `ZD_${label}`;
        const vz = Number(pickProp(el, 'vz') ?? 5.1);
        const izt = pickProp(el, 'izt') ?? '5m';
        lines.push(`D${label} ${nets[0]} ${nets[1]} ${model}`);
        extraModels.push(`.model ${model} D(BV=${vz} IBV=${izt})`);
        break;
      }
      case CircuitElementType.DIODE_LED: {
        const model = `LED_${label}`;
        const vf = Number(pickProp(el, 'vf') ?? 2);
        const rs = Math.max(0.1, vf / 0.02);
        lines.push(`D${label} ${nets[0]} ${nets[1]} ${model}`);
        extraModels.push(`.model ${model} D(IS=1e-18 N=2 RS=${rs})`);
        break;
      }
      case CircuitElementType.DIODE_SCHOTTKY: {
        const model = `SD_${label}`;
        const bv = Number(pickProp(el, 'bv') ?? 40);
        lines.push(`D${label} ${nets[0]} ${nets[1]} ${model}`);
        extraModels.push(`.model ${model} D(IS=1e-9 N=1.05 RS=0.2 BV=${bv})`);
        break;
      }
      ...
      case CircuitElementType.TRANSISTOR_PNP: {
        ...
        const c = nets[1];
        const e = nets[2];
        const isNpn = el.type === CircuitElementType.TRANSISTOR_NPN;
        const model = `${isNpn ? 'NPN' : 'PNP'}_${label}`;
        const precision = (pickProp(el, 'modelPrecision') as string) || 'standard';
        const beta = Number(pickProp(el, 'beta') ?? 100);
        if (precision === 'simple') {
          extraModels.push(`.model ${model} ${isNpn ? 'NPN' : 'PNP'}(IS=1e-14 BF=${Math.max(20, beta)})`);
        } else if (precision === 'accurate') {
          extraModels.push(`.model ${model} ${isNpn ? 'NPN' : 'PNP'}(IS=1e-14 BF=${Math.max(20, beta)} VAF=100 IKF=0.1 RC=0.5 CJC=4p CJE=5p TF=300p TR=10n)`);
        } else if (isNpn) {
          extraModels.push(`.model ${model} NPN(IS=6.734f BF=416 VAF=74 IKF=66.78m RC=1 CJC=3.638p CJE=4.493p TF=301.2p)`);
        } else {
          extraModels.push(`.model ${model} PNP(IS=1.41f BF=180 VAF=18.7 IKF=80m RC=0.6 CJC=9.728p CJE=8.063p TF=300p)`);
        }
        lines.push(`Q${label} ${c} ${b} ${e} ${model}`);
        break;
      }
      case CircuitElementType.OPAMP:
        // 默认宏模型参数（可在属性面板编辑）
        lines.push(`E${label} ${nets[2]} 0 ${nets[0]} ${nets[1]} 1e5`);
        break;
      case CircuitElementType.AMMETER: {
  ...
  extraModels.forEach((model) => lines.push(model));
  lines.push('.end');
