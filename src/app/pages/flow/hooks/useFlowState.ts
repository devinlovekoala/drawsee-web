...
import {
  getNodesByConvId,
  createAiTask
} from "@/api/methods/flow.methods.ts";
import {AiTaskType, NodeToUpdate, XYPosition} from "@/api/types/flow.types.ts";
...
function useFlowState(convId?: number) {
  ...
  // 用户ID
  const currentUserId = 1; // 简化处理，假设当前用户ID为1
          ...
          setTimeout(() => {
            setElements(({nodes, edges}) => {
              ...
              if (lastFocusNodeId.current) {
                setTimeout(() => {
                  executeFitView([lastFocusNodeId.current], 200);
                }, 200);
              }
              return {
                nodes: layoutedNodes,
  ...
  }, [adjustViewportToShowLatestContent, convId, executeLayout, executeFitView, handleTitleUpdate]);
  ...
  // 添加聊天任务
  const addChatTaskWithPolling = useCallback(async (
    convId: number, 
  ...
  return {
    ...
    chat,
    setElements,
    addChatTask: addChatTaskWithPolling
  };
}
