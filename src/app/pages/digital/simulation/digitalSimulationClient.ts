...
const DEFAULT_ENDPOINT = import.meta.env.VITE_DIGITAL_SIM_API_URL || 'http://localhost:3002/simulate/digital';
...
export const runDigitalSimulation = async (design: CircuitDesign): Promise<DigitalSimulationResult> => {
  ...
  if (!response.ok) {
    throw new Error(`数字仿真后端请求失败：${response.status}`);
  }

  const payload = await response.json();
  if (!payload.success) {
    throw new Error(payload.error || '数字仿真失败');
  }
