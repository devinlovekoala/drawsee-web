...
import { ConversationVO } from "@/api/types/flow.types.ts";
...
export function AppSidebar({ ...props }: React.ComponentProps<typeof Sidebar>) {
  const [conversations, setConversations] = React.useState<ConversationVO[]>([]);
  ...
  const handleNewConversation = async () => {
    try {
      const response = await createAiTask(0, 0, "");
